...
This crate exposes the raw `tachyon.h` C API via `bindgen`-generated bindings.
It is not intended for direct use — use [`tachyon-ipc`](https://crates.io/crates/tachyon-ipc) instead.

## What this crate provides

- `tachyon_bus_t`, `tachyon_msg_view_t` — opaque and layout types
- `tachyon_bus_listen`, `tachyon_bus_connect`, `tachyon_bus_destroy`
- `tachyon_acquire_tx`, `tachyon_commit_tx`, `tachyon_flush`
...
The C++ core (`arena.cpp`, `shm.cpp`, `tachyon_c.cpp`, `transport_uds.cpp`) is compiled
at build time via the `cc` crate. Requires GCC 14+ or Clang 17+.
