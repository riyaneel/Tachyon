...
class NativeBusHandle implements BusHandle<Buffer> {
	...
	/** Exposes the raw native binding for cross-binding wiring (e.g. StarBus). */
	public get native(): NativeBinding {
		return this.#handle;
	}
}
...
export class Bus extends BusBase<Buffer> {
	...
	/** @internal Raw native binding, consumed by {@link StarBus.create}. */
